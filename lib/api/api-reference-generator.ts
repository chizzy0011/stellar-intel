    ...
    for (const { method, path, op } of grouped[tag] ?? []) {
      ...
      if (op.parameters && op.parameters.length > 0) {
        ...
        for (const param of op.parameters) {
          ...
          const desc = param.description ? escapeMarkdownTableCell(param.description) : '-';
          lines.push(
          ...
          if (mediaType.schema) {
            ...
            if (props.length > 0) {
              ...
              for (const prop of props) {
                ...
                const desc = prop.description ? escapeMarkdownTableCell(prop.description) : '-';
      ...
      if (op.responses) {
        ...
        for (const [code, resp] of Object.entries(op.responses)) {
          const desc = resp.description ? escapeMarkdownTableCell(resp.description) : '-';
